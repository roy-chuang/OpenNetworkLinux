             ...
             char *buf)
{
    ...
    if (data->valid) {
        switch (attr->index) {
			...
			{
				u32 duty_cycle = reg_val_to_duty_cycle(data->reg_val[FAN_DUTY_CYCLE_PERCENTAGE]);
				u32 rpm_table[] = {0, 1959, 3945, 5464, 7257, 9108, 10780, 12636, 14464, 16332, 18919};
				ret = sprintf(buf, "%u\n", rpm_table[duty_cycle/10]);
				break;
			}
			...
			{
				u32 duty_cycle = reg_val_to_duty_cycle(data->reg_val[FAN_DUTY_CYCLE_PERCENTAGE]);
				u32 rpm_table[] = {0, 1660, 3275, 4670, 6129, 7510, 8925, 10437, 11965, 13150, 14702};
				ret = sprintf(buf, "%u\n", rpm_table[duty_cycle/10]);
				break;
			}
            case FAN1_PRESENT:
